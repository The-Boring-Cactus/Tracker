...
const loadTab = async (tab) => {
  ...
  const endpoints = {
    ...
    'versions': `http://127.0.0.1:8000/projects/${projectId}/versions`
  }
...
const createEntityBasic = async () => {
  ...
  if (activeTab.value === 'versions') {
    ...
    if (!name) return
    endpoint = `http://127.0.0.1:8000/projects/${projectId}/versions`
...
<template>
  <div class="min-h-screen bg-slate-50 flex flex-col text-slate-800">
    ...
    <div v-else class="flex-1 flex overflow-hidden">
      <!-- Sidebar -->
      <aside v-if="isSidebarOpen" class="w-64 bg-white border-r border-slate-200 p-4 flex flex-col flex-shrink-0 overflow-y-auto transition-all duration-300">
        ...
        <div class="space-y-1 flex-1">
          ...
          <div v-for="ws in workspaces" :key="ws.id" class="flex flex-col">
            ...
            <div v-if="expandedWorkspaces.has(ws.id)" class="ml-6 mt-1 space-y-1 border-l border-slate-200 pl-2">
              ...
              <button v-else v-for="proj in projectsByWorkspace[ws.id]" :key="proj.id" @click="goToAnotherProject(proj.id)" :class="['w-full text-left px-2 py-1.5 rounded-md text-xs font-medium transition truncate', proj.id == projectId ? 'bg-sky-100 text-sky-700' : 'text-slate-500 hover:text-sky-600 hover:bg-sky-50']">
                {{ proj.name }}
              </button>
            </div>
          </div>
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <button @click="triggerCreate" class="py-2 px-4 bg-sky-600 hover:bg-sky-500 text-white rounded-lg transition font-medium shadow-sm flex items-center">
      ...
      <div v-else-if="activeTab === 'board'" class="flex overflow-x-auto space-x-6 pb-4 h-[calc(100vh-280px)] items-start">
        <div v-for="status in statuses" :key="status" 
             class="w-80 flex-shrink-0 bg-slate-100/70 rounded-xl p-4 flex flex-col max-h-full border border-slate-200 shadow-inner"
             @dragover.prevent
          ...
          <div class="flex-1 overflow-y-auto space-y-3 pr-1 pb-2">
            <div v-for="issue in getIssuesByStatus(status)" :key="issue.id"
                 ...
                 class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 cursor-grab active:cursor-grabbing hover:border-sky-400 hover:shadow-md transition group flex flex-col min-h-[120px]">
              ...
              <div class="grid grid-cols-[auto_1fr] gap-x-3 items-center mb-2 border-b border-slate-100 pb-2">
                 <span class="text-xs font-semibold text-slate-400 w-12 shrink-0">#{{ issue.id }}</span>
                 <span class="text-xs font-medium text-slate-600 truncate">Task ID</span>
              </div>

              <div class="grid grid-cols-[auto_1fr] gap-x-3 items-center mb-2 border-b border-slate-100 pb-2">
                 <span class="text-xs font-semibold text-slate-400 w-12 shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                 </span>
                 <div class="flex items-center">
                   <div v-if="issue.assignee_id" class="w-6 h-6 rounded-full bg-sky-100 text-sky-700 flex items-center justify-center font-bold text-[10px] uppercase shadow-sm border border-sky-200 mr-1" :title="users.find(u => u.id === issue.assignee_id)?.username">
                      {{ getInitials(users.find(u => u.id === issue.assignee_id)?.username) }}
                   </div>
                   <div v-else class="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 font-bold text-[10px] border border-slate-200 shadow-sm mr-1" title="Unassigned">--</div>
                   <div class="w-6 h-6 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 font-bold text-[12px] border border-slate-200 border-dashed shadow-sm cursor-pointer hover:bg-slate-100 transition">+</div>
                 </div>
              </div>

              <div class="grid grid-cols-[auto_1fr] gap-x-3 items-center mb-3">
                 <span :class="['text-xs font-semibold w-12 shrink-0 transition', isOverdue(issue.end_date) ? 'text-red-500' : 'text-slate-400']">
              ...
              <div class="mt-auto bg-slate-50 rounded-lg p-3 flex flex-col justify-between items-start border border-slate-100">
                ...
                <div class="w-full flex justify-end">
                   <span class="text-xs font-bold text-slate-600 bg-white px-2 py-0.5 rounded shadow-sm border border-slate-200">{{ issue.estimated_hours ? `${issue.estimated_hours}h` : '--' }}</span>
                </div>
              </div>
      ...
      <div v-else-if="activeTab === 'issues'" class="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden flex flex-col">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-slate-200 text-sm text-left">
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
              <tr v-for="item in paginatedTreeData" :key="item.id" @click="goToIssueDetails(item.id)" class="hover:bg-slate-50 cursor-pointer transition group">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-slate-600">
                  <div class="flex items-center space-x-2">
                    <div v-if="item.assignee_id" class="w-6 h-6 rounded-full bg-sky-100 text-sky-700 flex items-center justify-center font-bold text-[10px] uppercase border border-sky-200">
                       {{ getInitials(users.find(u => u.id === item.assignee_id)?.username) }}
                    </div>
                    <div v-else class="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 font-bold text-[10px] border border-slate-200">--</div>
        ...
        <!-- Pagination -->
        <div class="border-t border-slate-200 bg-white px-4 py-3 flex items-center justify-between sm:px-6">
      ...
      <!-- Gantt Chart Layout -->
      <div v-else-if="activeTab === 'gantt'" class="bg-white shadow-sm border border-slate-200 rounded-xl overflow-x-auto p-4 flex flex-col min-h-[500px]">
      ...
      <div v-else-if="activeTab !== 'wiki' && activeTab !== 'gantt'" class="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden">
        <ul role="list" class="divide-y divide-slate-200">
          <li v-for="item in flatTreeData" :key="item.id" class="transition group flex flex-col">
            <div @click="toggleExpand(item.id)" :style="{ paddingLeft: (item.depth ? item.depth * 24 + 24 : 24) + 'px' }" class="py-4 pr-6 hover:bg-slate-50 cursor-pointer flex items-center justify-between border-l-4 border-transparent hover:border-sky-500">
              
              <div class="flex items-center space-x-3 w-full">
                <!-- Tree Toggle Icon -->
                <div v-if="item.children && item.children.length > 0" @click="toggleTreeRow(item.id, $event)" class="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-sky-600 bg-white border border-slate-200 rounded cursor-pointer shadow-sm">
            ...
            <div v-if="expandedItemId === item.id" class="px-6 py-6 bg-slate-50 border-t border-slate-100 flex flex-col md:flex-row gap-8 shadow-inner" :style="{ marginLeft: (item.depth ? item.depth * 24 : 0) + 'px' }">
      ...
      <div v-else-if="activeTab === 'wiki'" class="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden flex flex-col">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-slate-200 text-sm text-left">
            <thead class="bg-slate-50">
              <tr>
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
              <tr v-for="item in tabData" :key="item.id" @click="goToWikiDetails(item.id)" class="hover:bg-slate-50 cursor-pointer transition group">
                <td class="px-4 py-3 whitespace-nowrap text-slate-800 font-medium group-hover:text-sky-600 transition">{{ item.title }}</td>
                <td class="px-4 py-3 whitespace-nowrap text-slate-600">
                  <div class="flex items-center space-x-2">
                    <div class="w-6 h-6 rounded-full bg-sky-100 text-sky-700 flex items-center justify-center font-bold text-[10px] uppercase border border-sky-200">
                ...
                <td class="px-4 py-3 whitespace-nowrap">
                  <div class="flex items-center space-x-1">
                    ...
                    <span v-else class="text-xs text-slate-400 italic">No tags</span>
                  </div>
                </td>
    ...
    <!-- Create Issue Modal -->
    <div v-if="showCreateModal" class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
        ...
        <div class="p-6 overflow-y-auto flex-1 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
              ...
              <input type="text" v-model="newIssue.title" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="E.g. Fix login bug" required />
            </div>
            
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Category</label>
              <div class="flex space-x-2">
                <select v-model="newIssue.category" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500">
            ...
            <div>
              ...
              <select v-model="newIssue.assignee_id" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500">
            ...
            <div>
              ...
              <select v-model="newIssue.parent_id" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500">
            ...
            <div class="md:col-span-2">
              ...
              <div class="w-full bg-white border border-slate-300 rounded-lg p-2 flex flex-wrap gap-2 focus-within:ring-2 focus-within:ring-sky-500 transition-all">
            ...
            <div>
              ...
              <input type="date" v-model="newIssue.start_date" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500" />
            </div>

            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">End Date (Due Date)</label>
              <input type="date" v-model="newIssue.end_date" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500" />
            </div>

            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Estimated Time (Hours)</label>
              <input type="number" step="0.5" v-model="newIssue.estimated_hours" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="e.g. 2.5" />
            </div>
          ...
          <div>
            ...
            <div class="border border-slate-300 rounded-lg overflow-hidden bg-white">
               <QuillEditor theme="snow" v-model:content="newIssue.description" contentType="html" toolbar="full" :options="editorOptions" class="h-64" />
            </div>
          </div>
        </div>
        
        <div class="p-6 border-t border-slate-200 bg-slate-50 flex justify-end space-x-3 rounded-b-xl">
          <button @click="showCreateModal = false" class="py-2 px-4 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 rounded-lg transition font-medium">Cancel</button>
    ...
    <!-- Create Wiki Modal -->
    <div v-if="showWikiModal" class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
        <div class="p-6 border-b border-slate-200 flex justify-between items-center">
          <h2 class="text-xl font-bold text-slate-800">Create New Wiki Page</h2>
          <button @click="showWikiModal = false" class="text-slate-400 hover:text-slate-600 transition">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
          </button>
        </div>
        ...
        <div class="p-6 overflow-y-auto flex-1 space-y-6">
          <div>
            ...
            <input type="text" v-model="newWiki.title" class="w-full bg-white border border-slate-300 rounded-lg py-2 px-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="E.g. Getting Started" required />
          </div>
          
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Content</label>
            <div class="border border-slate-300 rounded-lg overflow-hidden bg-white">
               <QuillEditor theme="snow" v-model:content="newWiki.content" contentType="html" toolbar="full" :options="editorOptions" class="h-[300px]" />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="flex items-center space-x-2 mb-2">
              <input type="text" v-model="wikiTagInput" @keydown.enter.prevent="handleAddWikiTag" placeholder="e.g. documentation, process" class="flex-1 bg-white border border-slate-300 rounded-lg py-2.5 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 transition" />
              <button @click="handleAddWikiTag" type="button" class="py-2.5 px-5 bg-slate-100 border border-slate-200 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-200 transition">Add Tag</button>
            </div>
        ...
        <div class="p-6 border-t border-slate-200 bg-slate-50 flex justify-end space-x-3 rounded-b-xl">
          <button @click="showWikiModal = false" class="py-2 px-4 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 rounded-lg transition font-medium">Cancel</button>
