...
<template>
  <div class="min-h-screen bg-slate-50 flex flex-col text-slate-800">
      ...
      <div class="flex-1 min-w-0 space-y-8">
        ...
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <div class="flex items-center space-x-3 mb-4">
            ...
            <span class="px-3 py-1 rounded text-xs font-bold uppercase tracking-wide bg-slate-100 text-slate-600 border border-slate-200">
              {{ issue.category }}
            </span>
        ...
        <!-- Description -->
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Description</h3>
        ...
        <!-- Attachments -->
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2 flex items-center justify-between">
            Attachments
            <label class="cursor-pointer py-1 px-3 bg-slate-50 border border-slate-200 rounded-lg shadow-sm text-xs font-bold text-sky-600 hover:bg-sky-50 hover:border-sky-200 transition inline-flex items-center">
          ...
          <div v-if="issue.attachments && issue.attachments.length > 0" class="flex flex-wrap gap-4">
            <a v-for="att in issue.attachments" :key="att.id" :href="`http://127.0.0.1:8000${att.file_path}`" target="_blank" class="flex items-center p-3 bg-slate-50 border border-slate-200 rounded-xl hover:bg-sky-50 hover:border-sky-300 hover:shadow-md transition group">
              <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center mr-3 shadow-sm text-sky-500 group-hover:text-sky-600">
              ...
              <div class="flex flex-col">
                ...
                <span class="text-xs text-slate-400">Uploaded by {{ users.find(u => u.id === att.uploader_id)?.username }}</span>
              </div>
            </a>
          </div>
        ...
        <!-- Comments -->
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-6 border-b border-slate-100 pb-2">Comments ({{ issue.comments?.length || 0 }})</h3>
          ...
          <div class="space-y-6 mb-8 max-h-[500px] overflow-y-auto pr-2">
            <div v-for="comment in issue.comments" :key="comment.id" class="flex space-x-4">
              ...
              <div class="flex-1 min-w-0 bg-slate-50 p-5 rounded-2xl rounded-tl-none border border-slate-100 shadow-sm">
          ...
          <div class="border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-sky-500 transition shadow-sm bg-white">
            <QuillEditor theme="snow" v-model:content="commentContent" contentType="html" toolbar="full" :options="editorOptions" class="min-h-[120px]" placeholder="Add a comment..." />
            <div class="bg-slate-50 p-3 border-t border-slate-200 flex justify-end">
              <button @click="addComment" class="py-2 px-6 bg-sky-600 hover:bg-sky-500 text-white rounded-lg text-sm font-bold transition shadow-sm">Post Comment</button>
            </div>
          </div>
        </div>
      ...
      <div class="w-full lg:w-80 flex-shrink-0 space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
          <div>
            ...
            <select @change="updateStatus($event.target.value)" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-3 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white transition">
              <option v-for="s in statuses" :key="s" :value="s" :selected="issue.status === s">{{ s }}</option>
            </select>
          </div>
          
          <div>
            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Assignee</label>
            <select @change="updateAssignee($event.target.value)" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-3 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white transition">
          ...
          <div>
            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Reporter</label>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
          <h3 class="text-sm font-bold text-slate-800 border-b border-slate-100 pb-2">Schedule & Estimates</h3>
          
          <div class="grid grid-cols-2 gap-4">
            <div>
              <span class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Start Date</span>
              <span class="text-sm font-medium text-slate-700">{{ issue.start_date ? new Date(issue.start_date).toLocaleDateString() : 'None' }}</span>
            </div>
            <div>
              <span class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Due Date</span>
            ...
            <div class="col-span-2">
              <span class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Estimated Time</span>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
          <h3 class="text-sm font-bold text-slate-800 border-b border-slate-100 pb-2">Time Tracking</h3>
          
          <div class="space-y-4">
            <!-- Reporter Time -->
            <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col items-center justify-center text-center relative overflow-hidden group">
              <div class="absolute inset-0 bg-gradient-to-br from-transparent to-slate-100 opacity-50"></div>
              <span class="relative text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Reporter Logged</span>
              ...
              <button @click="openTimeModal('reporter')" class="relative text-xs font-bold bg-white border border-slate-200 hover:border-sky-300 hover:text-sky-600 px-4 py-1.5 rounded-full shadow-sm transition">Grab Time</button>
            </div>
            
            <!-- Assignee Time -->
            <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col items-center justify-center text-center relative overflow-hidden">
              <div class="absolute inset-0 bg-gradient-to-br from-transparent to-sky-50 opacity-30"></div>
              <span class="relative text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Assignee Logged</span>
              ...
              <button @click="openTimeModal('assignee')" class="relative text-xs font-bold bg-white border border-slate-200 hover:border-sky-300 hover:text-sky-600 px-4 py-1.5 rounded-full shadow-sm transition">Grab Time</button>
            </div>
          </div>
        </div>
    ...
    <!-- Time Logging Modal -->
    <div v-if="showTimeModal" class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm flex flex-col transform transition-all">
        <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-xl">
          ...
          <button @click="showTimeModal = false" class="text-slate-400 hover:text-slate-600 transition">
        ...
        <div class="p-6">
          ...
          <div class="relative">
            <input type="number" step="0.1" v-model="timeToLog" class="w-full bg-white border border-slate-300 rounded-lg py-2.5 px-4 text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition" placeholder="e.g. 1.5" @keydown.enter="submitTime" />
            <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
              <span class="text-slate-400 font-semibold">hrs</span>
            </div>
          </div>
        </div>
        
        <div class="p-5 border-t border-slate-100 bg-slate-50 flex justify-end space-x-3 rounded-b-xl">
          <button @click="showTimeModal = false" class="py-2 px-4 bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 rounded-lg transition font-medium text-sm">Cancel</button>
          <button @click="submitTime" :disabled="!timeToLog" class="py-2 px-6 bg-sky-600 hover:bg-sky-500 disabled:opacity-50 text-white rounded-lg transition font-bold text-sm shadow-sm">Save Log</button>
        </div>
      </div>
    </div>
